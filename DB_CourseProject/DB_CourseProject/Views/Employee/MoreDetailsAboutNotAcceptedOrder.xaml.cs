...
{
    public partial class MoreDetailsAboutNotAcceptedOrder : Window
    {
        private int orderId;
        private Info detailsOfOrder;

        public MoreDetailsAboutNotAcceptedOrder(int orderId)
        {
            InitializeComponent();
        ...
        private void LoadOrderDetails()
        {
            try
            {
                using (OracleConnection connection = new OracleConnection(OracleDatabaseConnection.adminConn))
                {
                    ...
                    using (OracleCommand getOrderByOrderIdCommand = new OracleCommand("getOrderByOrderIdWithoutEID", connection))
                    {
            ...
            try
            {
                using (OracleConnection connection = new OracleConnection(OracleDatabaseConnection.adminConn))
                {
                    ...
                    using (OracleCommand getTypeByModelCommand = new OracleCommand("getTypeByModel", connection))
                    {
                        ...
                        getTypeByModelCommand.ExecuteNonQuery();
                        OracleDataReader reader = ((OracleRefCursor)getTypeByModelCommand.Parameters["curs"].Value).GetDataReader();
                        string Type = "";
                        if (reader.Read())
                        {

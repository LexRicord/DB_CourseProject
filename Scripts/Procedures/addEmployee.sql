...
in_secondname in Employees.SecondName%TYPE, in_login in Employees.Login%TYPE, in_ephonenumber EMPLOYEES.EPHONENUMBER%Type,
in_password in Employees.Password%TYPE, in_Post in Employees.Post%TYPE)
IS
user_exists number;
...
begin
    if in_name is null or in_surname is null or in_secondname is null or in_login is null
           or in_password is null or in_Post is null or in_ephonenumber is null
    ...
    if (user_exists != 0 ) then raise curr_user_exists;
        ...
        else insert into Employees (Name, Surname, SecondName, Login, Password, Post,EPHONENUMBER)
        values (in_name, in_surname, in_secondname, in_login,encode_pass, in_Post, in_ephonenumber);
    end if;
    exception

...
(
    ...
    Balance NUMBER(7,2),
    Role NVARCHAR2(30),
    CONSTRAINT check_client_email CHECK (REGEXP_LIKE(Email, '.*@.*\..*')),
    CONSTRAINT check_passport_number CHECK (
        PassportNumber IS NULL OR
        REGEXP_LIKE(PassportNumber, 'AB[0-9]{7}') OR
        REGEXP_LIKE(PassportNumber, 'BM[0-9]{7}') OR
        REGEXP_LIKE(PassportNumber, 'HB[0-9]{7}') OR
        REGEXP_LIKE(PassportNumber, 'KH[0-9]{7}') OR
        REGEXP_LIKE(PassportNumber, 'MC[0-9]{7}') OR
        REGEXP_LIKE(PassportNumber, 'KB[0-9]{7}') OR
        REGEXP_LIKE(PassportNumber, 'PP[0-9]{7}')
    ),
    CONSTRAINT check_cli_phonenumber CHECK (
        REGEXP_LIKE(PhoneNumber, '\+37529[0-9]{7}') OR
        REGEXP_LIKE(PhoneNumber, '\+37517[0-9]{7}') OR
        REGEXP_LIKE(PhoneNumber, '\+37533[0-9]{7}') OR
        REGEXP_LIKE(PhoneNumber, '\+37544[0-9]{7}')
    )
);
...
CREATE TABLE Employees (
    ...
    Surname NVARCHAR2(50) DEFAULT '-' NOT NULL,
    SecondName NVARCHAR2(50) DEFAULT '-' NOT NULL,
    EPhoneNumber NVARCHAR2(50),
    Password NVARCHAR2(2000) NOT NULL,
    Role NVARCHAR2(50) NOT NULL,
    CONSTRAINT check_employee_email CHECK (REGEXP_LIKE(Email, '.*@.*\..*')),
    CONSTRAINT check_empl_phonenumber CHECK (
        REGEXP_LIKE(EPhoneNumber, '\+37529[0-9]{7}') OR
        REGEXP_LIKE(EPhoneNumber, '\+37517[0-9]{7}') OR
        REGEXP_LIKE(EPhoneNumber, '\+37533[0-9]{7}') OR
        REGEXP_LIKE(EPhoneNumber, '\+37544[0-9]{7}')
    )
);

INSERT INTO Employees (Email, Name, Surname, SecondName, EPhoneNumber, Password, Role)
VALUES ('admin@mail.ru', 'Александр', 'Герман', 'Евгеньевич', '+375292383759', 'admin', 'admin');
...
(
    ...
    SumOfTransaction NUMBER(7, 2) NOT NULL DEFAULT 0,
